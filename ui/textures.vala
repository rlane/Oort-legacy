...
namespace Oort {
	...
	public class LaserBeamTexture : Texture {
		public LaserBeamTexture() {
			bind();
			glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
			glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
			...
			for (int y = 0; y < n; y++) {
				for (int x = 0; x < n; x++) {
					var i = 3*(n*y+x);
					var f = y < 250 ? 1.0 : (255-y)/5.0;
					data[i+0] = (uint8)(255*f*Math.sin(Math.PI*x/(n-1)));
					data[i+1] = 0; //(uint8)(0.5*255*f*Math.sin(Math.PI*x/(n-1)));
					data[i+2] = 0; //(uint8)(0.5*255*f*Math.sin(Math.PI*x/(n-1)));
				}
			}
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, n, n, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
			glBindTexture(GL_TEXTURE_2D, 0);
